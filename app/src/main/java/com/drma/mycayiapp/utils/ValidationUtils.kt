...
private fun isEnteredPasswordValid(context: Context, editText: EditText, resFieldName: Int, maxLength: Int, checkName: Boolean): Boolean {
    var isCorrect = false
    val p: Pattern
    if (checkName) {
        p = Pattern.compile("^[a-zA-Z][a-zA-Z0-9]{2," + (maxLength - 1) + "}+$")
    ...
    if (!isCorrect) {
        editText.error = String.format(context.getString(R.string.error_name_must_not_contain_special_characters_from_app_Password),
            context.getString(resFieldName),
            maxLength)
        return false
    } else {
...
fun isFoolNameValid(context: Context, editText: EditText): Boolean {
    return isEnteredTextValid(context, editText, R.string.field_name_user_fullname, MAX_FULLNAME_LENGTH, true)
}
fun isPasswordValid(context: Context, editText: EditText): Boolean {
    return isEnteredPasswordValid(context, editText, R.string.field_name_user_password, MAX_FULLNAME_LENGTH, true)
}
